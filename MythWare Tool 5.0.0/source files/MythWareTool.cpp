	...
	: QWidget(parent)
{
	...
	info_page_text->setText(MWT_VERSION);
...
void MythWareTool::process_stop_clicked()
{
	...
	return_check = mwtProcessControl(L"StudentMain.exe", MWT_SUSPEND_PROCESS); //挂起进程
	if (!return_check)
	{
		mwtmsgbox("挂起进程时遇到问题 进程可能没有被挂起", "错误", BTN_OK, ICON_ERROR);
	}
}
...
{
	bool return_check = false;
	return_check = mwtProcessControl(L"StudentMain.exe", MWT_RESUME_PROCESS); //恢复进程
	if (!return_check)
	{
		mwtmsgbox("无法恢复被挂起的进程 该进程可能不存在", "错误", BTN_OK, ICON_ERROR);
	}
}
...
{
	bool return_check = false;
	return_check = mwtProcessControl(L"StudentMain.exe", MWT_TERMINATE_PROCESS); //结束进程
	if (!return_check)
	{
...
void MythWareTool::re_process_clicked()
{
	...
	{
		mwtmsgbox("应用程序无法启动 原因: 路径无效 请重设学生端路径", "错误", BTN_OK, ICON_ERROR);
	}
...
{
	mwtProcessControl(L"StudentMain.exe", MWT_TERMINATE_PROCESS);
	int returned;
	QSettings ini("C:\\Program Files\\mwtconfig.ini", QSettings::IniFormat);
	QString path = ini.value("StudentMainpath/path").toString();

	if (!replacefile(path, "LibDeskMonitor.dll"))
	{
		returned = mwtmsgbox("选定的目录下LibDeskMonitor.dll可能不存在 是否重试?", "警告", BTN_YESNO, ICON_WARNING);
	}
	...
	if (returned == 1)
	{
		if (!replacefile(path, "LibDeskMonitor.dll"))
		{
			returned = mwtmsgbox("重试失败 请重设学生端路径", "错误", BTN_OK, ICON_ERROR);
		}
	}
...
int MythWareTool::MythWareToolMessageBox(const char msg_text[], const char title[], int btn, int icon_type)
{
	...
	if (icon_type == ICON_WARNING)
	{
		QLabel *icon = new QLabel;
		icon->setParent(msg);
		icon->setPixmap(QPixmap(":/icon/warning.png"));
		icon->setFixedSize(35, 35);
		icon->move(30, 33);
		icon->setScaledContents(true);
	}

	if (strlen(msg_text) > 30) //msgbox自动延长(x轴) 临时补丁
	{
		msg->setFixedSize(300 + strlen(msg_text) * 2, 150);
		line->setGeometry(0, 100, 300 + strlen(msg_text) * 2, 10);
		btn_ok->move(220 + strlen(msg_text) * 2, 110);
		btn_yes->move(120 + strlen(msg_text) * 2, 110);
		btn_no->move(220 + strlen(msg_text) * 2, 110);
	}
...
bool MythWareTool::replacefile(QString targetdir, QString filename)
{
	bool remove_check, copy_check;

	remove_check = QFile::remove( targetdir+ "\\"+filename);
	if (remove_check)
	{
		copy_check = QFile::copy(filename, targetdir + "\\" + filename);
		if (copy_check)
		{
			return true;
		}
...
bool MythWareTool::mwtProcessControl(const wchar_t* exename, int controlmode)
{
	DWORD processpid = 0;
	HANDLE hsnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	PROCESSENTRY32 processentry32;
	processentry32.dwSize = sizeof(PROCESSENTRY32);
	BOOL P32ret = Process32First(hsnap, &processentry32);
	while (P32ret)
	{
		if (_wcsicmp(processentry32.szExeFile, exename) == 0)
		{
			processpid = processentry32.th32ProcessID;
		}
		P32ret = Process32Next(hsnap, &processentry32);
	}
	CloseHandle(hsnap);
	
	if (controlmode == 11)
	{
		HANDLE threadsnap = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, processpid);
		THREADENTRY32 threadentry32;
		threadentry32.dwSize = sizeof(THREADENTRY32);
		BOOL T32ret = Thread32First(threadsnap, &threadentry32);
		while (T32ret)
		{
			if (threadentry32.th32OwnerProcessID == processpid)
			{
				HANDLE openThread = OpenThread(THREAD_SUSPEND_RESUME, FALSE, threadentry32.th32ThreadID);
				if (openThread)
				{
			...
			T32ret = Thread32Next(threadsnap, &threadentry32);
		}
	...
	if (controlmode == 13)
	{
		HANDLE hopenprocess = OpenProcess(PROCESS_TERMINATE, FALSE, processpid);
		if (hopenprocess)
		{
			TerminateProcess(hopenprocess, 0);
